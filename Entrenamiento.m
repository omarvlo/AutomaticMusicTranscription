...
base_MAPS = 0; %SE ELIGE LA BASE DE DATOS 
                %1 MAPS ENSTDkCl
                %0 Base de datos de Maestr�a
                %2 MAPS AkPnBsdf
resampleo=0;
reduce = 2; %determina en cuanto se reduce la Fs
            %por ejemplo 2 ser�a una reducci�n de Fs/2
...
% Filtros ERBs
num_filtros = 12;
frec_minima = 20; %60 Hz para el C2
...
% num_filtros = 5;
% banda =0; % En realidad es transcrip (y en este caso queremos ENTRENAR no transcribir)
% modelo=1; % 1: Propuesta: 2: M. Slaney 3:Greenwood
% 
% [AC4_ERB Propuesto] = filtros_ERB(fs,num_filtros,frec_minima,AC4,banda,modelo);
% 
% plot(Propuesto,'m','LineWidth',1.5)
% axis([0 2000 0 1.25])
% %t = title('Banco de filtros auditivos de M. Slaney')
...
% 
% axis([0 600 0 1.5])
% modelo=2; % 1: Propuesta: 2: M. Slaney 3:Greenwood
% 
% [AC4_ERB MSlaney] = filtros_ERB(fs,num_filtros,frec_minima,AC4,banda,modelo);
% 
% plot(MSlaney,'b--','LineWidth',1.5)
% axis([0 2000 0 1.25])
% t = title('Banco de 5 filtros auditivos')
% xlabel('Frecuencia (Hz)');
% ylabel('|H(w)|');
% t.FontSize = 22;
% ax = gca;
% ax.FontSize = 22;
% %legend('Propuesto','MSlaney')
% % lgd = legend;
% % lgd.FontSize = 20;
% hold on
% 
% modelo=3; % 1: Propuesta: 2: M. Slaney 3:Greenwood
% 
% [AC4_ERB Greenwood] = filtros_ERB(fs,num_filtros,frec_minima,AC4,banda,modelo);
% 
% plot(Greenwood,'r-.','LineWidth',1.5)
% axis([0 2000 0 1.25])
% t = title('Banco de 5 filtros auditivos')
% xlabel('Frecuencia (Hz)');
% ylabel('|H(w)|');
...
% plot(AE2)
% axis([0 500 0 180])
% t = title('Espectrograma de E2 original')
% %xlabel('Frecuencia (Hz)');
% ylabel('|H(w)|');
% t.FontSize = 20;
% ax = gca;
...
% plot(AE2_ERB)
% axis([0 500 0 180])
% t = title('Espectrograma de E2 filtrado')
% xlabel('Frecuencia (Hz)');
% ylabel('|H(w)|');
% t.FontSize = 20;
% ax = gca;
...
% plot(resp)
% axis([0 1500 0 20])
% t = title('Espectrograma de E2 y banco de filtros')
% xlabel('Frecuencia (Hz)');
% ylabel('|H(w)|');
% t.FontSize = 22;
% ax = gca;
...
% plot(AE2)
% axis([0 500 0 180])
% t = title('Espectrograma de la nota E2')
% xlabel('Frecuencia (Hz)');
% ylabel('|H(w)|');
% t.FontSize = 22;
% ax = gca;
...
transcrip =0; % En realidad es transcrip (y en este caso queremos ENTRENAR no transcribir)
modelo=1; % 1: Propuesta: 2: M. Slaney 3:Greenwood
...
save('12fil_88n_20hz_espectro_venterc.mat','numComp','numIter','paramSTFT','deltaF','deltaT','initW','numTemplateFrames', 'num_filtros','frec_minima');
