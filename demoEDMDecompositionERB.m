...
% tam = length(xEffects);
% xEffects = xEffects(1:round(0.5*tam));
% xBass = xBass(1:round(0.5*tam));
% xMelody = xMelody(1:round(0.5*tam));
% xDrums = xDrums(1:round(0.5*tam));
...
if repr_ERB == 1
    ATr_ERB = cocleograma(xTr,fs,num_filtros,frec_minima,0.023);
    
%     ATr_ERB = simplifica_ERB(ATr_ERB, 0.023, fs); % se reducen los coeficientes de los filtros de acuerdo con Vicent(06)
%     ATr_ERB = ATr_ERB';
    
    [numBinsTr, numFramesTr] = size(ATr_ERB);
    
    AEffects_ERB = cocleograma(xEffects,fs,num_filtros,frec_minima,0.023);
%     AEffects_ERB = simplifica_ERB(AEffects_ERB, 0.023, fs);
%     AEffects_ERB = AEffects_ERB';
    ABass_ERB = cocleograma(xBass,fs,num_filtros,frec_minima,0.023);
%     ABass_ERB = simplifica_ERB(ABass_ERB, 0.023, fs);
%     ABass_ERB = ABass_ERB';
    AMel_ERB = cocleograma(xMelody,fs,num_filtros,frec_minima,0.023);
%     AMel_ERB = simplifica_ERB(AMel_ERB, 0.023, fs); 
%     AMel_ERB = AMel_ERB';
    ADrums_ERB = cocleograma(xDrums,fs,num_filtros,frec_minima,0.023);
%     ADrums_ERB = simplifica_ERB(ADrums_ERB, 0.023, fs); 
%     ADrums_ERB = ADrums_ERB';
    
    [numBinsBass, numFramesBass] = size(ABass_ERB);
...
[numBinsBass, numFramesBass] = size(ABass);
...
% NMFD core method
 [nmfdW, nmfdH, nmfdV, divKL] = NMFD(ATr_ERB, paramNMFD);
%[nmfdW, nmfdH, nmfdV, divKL] = NMFD(ATr, paramNMFD);
% %% visualize
paramVis.deltaT = deltaT;
paramVis.deltaF = deltaF;
paramVis.logComp = 1e5;
fh1 = visualizeComponentsNMF(ATr_ERB, nmfdW, nmfdH, nmfdV, paramVis);

%% save result
saveas(fh1,[outPath,'LSDDM_EM_ERB_88_si.png']);
